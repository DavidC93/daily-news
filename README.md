...
## פרסום מהדורה מהטלפון

`/admin/publish` (אותה סיסמה של `/admin`): מדביקים את ה-JSON של המהדורה, לוחצים "בדוק מהדורה", רואים תצוגה מקדימה ולוחצים "פרסם מהדורה".

- סימוני Markdown (למשל ```` ```json ````) וטקסט מסביב, כמו בהעתקה מ-ChatGPT, מנוקים אוטומטית.
- הבדיקה והשמירה נעשות בצד השרת (`/api/admin/publish`), בהרשאת ה-session של הניהול. מפתח ה-API לא נשלח לדפדפן.
- אם כבר קיימת מהדורה לאותו תאריך, היא לא נדרסת.
...
4. אל תסמנו את המשתנים כ-Secret עם הקשר (context) של "All": ‏Netlify לא שומר אותם, ולא מציג שגיאה. האתר ייכשל בכל עמוד. אחרי כל שינוי במשתנים צריך בנייה מחדש (Trigger deploy).
